...
$(function() {
    if($('#btn_buscar_docentes_evaluacion_revisar').length){
        $('#btn_buscar_docentes_evaluacion_revisar').on('click', function() {
            data_ajax(site_url+'/evaluacion_docente/buscar_docentes_evaluacion_revisar', '#form_busqueda_docentes_evaluar', '#div_result_docentes_evaluacion');
        });
    }
    data_ajax(site_url+'/evaluacion_docente/buscar_docentes_evaluacion_revisar', '#form_busqueda_docentes_evaluar', '#div_result_docentes_evaluacion');
});
...
function funcion_buscar_docentes_evaluar() {
    var path = site_url + '/evaluacion_docente/buscar_docentes_evaluacion_revisar/0';
...
function runScript_busqueda_val(e) {
    if (e.keyCode == 13) {
        $("#btn_buscar_docentes_evaluacion_revisar").trigger("click");//Dispara el onclic del botón buscar validador en el sied
        return false;
    }
}
...
function funcion_ver_validacion_empleado(element) {
    ...
    data_ajax_post(site_url + '/evaluacion_docente/seccion_revisar_index', null, '#select_perfil_validar_evaluacion', obj_post);
}
